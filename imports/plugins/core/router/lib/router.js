...
Router.initPackageRoutes = (options) => {
  ...
  // If app is running in IDENTITY_PROVIDER_MODE, apply only IDP routes
  const idpPackage = packages.find((pkg) => pkg.name === "reaction-hydra-oauth");
  if (idpPackage && idpPackage.enabled) defaultRouteDefinitions = [];
